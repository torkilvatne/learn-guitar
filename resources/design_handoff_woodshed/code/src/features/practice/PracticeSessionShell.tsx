...
export interface PracticeSessionShellProps {
  title: string
  /** The actual question, e.g. "How far is the arrow?" — shown above the exercise. */
  question?: string
  description: ReactNode
  score?: PracticeSessionScore
  /** One entry per answered round: true = clean first try. Drives the dots. */
  results?: boolean[]
  roundTarget?: number
  children: ReactNode
}
...
/**
 * Changes:
 *   - Rounded sheet on a plain backdrop; the old version was a hard-bordered box with
 *     a heavy shadow over a blurred, 90%-opacity page.
 *   - Header is Leave + progress dots. The title moves into the prompt block, where
 *     it sits above a real question sentence.
 *   - The start screen gets the same rounded, centred treatment.
 */
